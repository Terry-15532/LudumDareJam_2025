...
public enum FoodCategory
{
    Lemon = 0, Mango, Eggs, Banana, Milk, Cola, Pepsi, Soda,
    Salad, Cheese, Sandwich, RiceBall, DonutBlue, Watermelon, DonutPink
}
...
public class Food : MonoBehaviour
{
    ...
    public static readonly Color[] outlineColors = new[]
    {
        new Color(0.8f, 0.7f, 0.1f), new Color(1f, 0.6f, 0.2f), new Color(1f, 1f, 0.9f),
        new Color(1f, 0.9f, 0.3f), new Color(1f, 1f, 1f), new Color(0.7f, 0f, 0f),
        new Color(0f, 0.4f, 0.65f), new Color(0.6f, 0.8f, 0.2f), new Color(0.4f, 0.8f, 0.4f),
        new Color(1f, 0.85f, 0.4f), new Color(0.8f, 0.6f, 0.4f), new Color(0.9f, 0.8f, 0.6f),
        new Color(0.6f, 0.95f, 1f), new Color(0.2f, 0.6f, 0.2f), new Color(0.95f, 0.6f, 0.6f),
    };
    ...
    public float maxWanderDistance = 1f;
    private Vector3 wanderPosition;
    private Vector3 currentVelocity = Vector3.zero;
    private Vector3 targetDirection = Vector3.zero;
    ...
    void Update()
    {
        if (!controlling || qte.getQTEStarted()) return;
        ...
        switch (mode)
        {
            case MovementMode.Keyboard:
                HandleKeyboard();
                break;
            case MovementMode.MouseVelocity:
                HandleMouseVelocity();
                break;
            case MovementMode.MousePosition:
                HandleMousePosition();
                break;
        }
    ...
    void HandleMouseVelocity()
    {
        ...
        Vector3 direction = new Vector3(mouseX, mouseY, 0f).normalized;
        float mouseSpeed = new Vector2(mouseX, mouseY).magnitude / Time.deltaTime;
        float cappedSpeed = Mathf.Min(mouseSpeed * 0.01f, maxMouseSpeed);
        ...
        {
            Vector3 toCamera = (Camera.main.transform.position - transform.position).normalized;
            transform.position += toCamera * zMoveSpeed * Time.deltaTime;
        }
    ...
    void HandleMousePosition()
    {
        ...
        if (movementPlane.Raycast(ray, out float enter))
        {
            ...
            if (toMouse.magnitude > stopThreshold)
            {
                Vector3 targetVirtualVelocity = toMouse.normalized * maxMouseSpeed;
                virtualVelocity = Vector3.MoveTowards(virtualVelocity, targetVirtualVelocity, mouseAcceleration * Time.deltaTime);
                virtualPoint += virtualVelocity * Time.deltaTime;
        ...
        if (isWandering)
        {
            float dist = Vector3.Distance(transform.position, virtualPoint);
            if (dist > maxWanderDistance)
            {
                currentVelocity = Vector3.Lerp(currentVelocity, (virtualPoint - transform.position).normalized * maxWanderSpeed, Time.deltaTime / transitionDuration);
            }
            else
            {
                currentVelocity = Vector3.Lerp(currentVelocity, targetDirection * maxWanderSpeed, Time.deltaTime / transitionDuration);
    ...
    public void OnMouseEnter()
    {
        if (!selected) sr.material.SetFloat(emissionID, 2.5f);
    }

    public void OnMouseExit()
    {
        if (!selected) sr.material.SetFloat(emissionID, 0);
    }
